...
int main(){
    ...
    vector<int> a(3);
    int res = min({prog.size(),mat.size(),sport.size()});
    cout << res << endl;
    int dif = 0;
    // printar(prog); printar(mat); printar(sport);


    if(prog.size() > res){
        dif = prog.size() - res;
        prog.erase(prog.begin()+(dif-1));
        // cout << prog[0] << endl;
    }
    if(mat.size() > res){
        dif = mat.size() - res;
        mat.erase(mat.begin()+dif);
    }
    if(sport.size() > res){
        dif = sport.size() - res;
        sport.erase(sport.begin()+dif);
    }
    for(int k = 0; k < res; k++){
        cout << prog[k] << " " << mat[k] << " " << sport[k] << endl;
